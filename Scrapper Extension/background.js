document.addEventListener('DOMContentLoaded', function () {
  chrome.tabs.query({ active: true, lastFocusedWindow: true }, tabs => {
    ...
    else if (url != "https://www.guru.com/work/" && url != "https://www.freelancer.com/search/projects/?ngsw-bypass=&w=f") {
      url = url.split('?');
      url = url[0]
    }
    ...
    if (url == "https://www.freelancer.com/search/projects/?ngsw-bypass=&w=f") {
      ...
      document.getElementById('frating').style.display = 'block';
    }
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.message == 'started') {
    var check = document.getElementById('rating');
