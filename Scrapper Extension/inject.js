...
var status = 0;
var frating = 'false';
chrome.runtime.sendMessage({ message: "started" }, async response => {
...
if(frating == 'true') {
  var rating = allPosts[i].getElementsByClassName('Rating--labeled')[0];
  if(!rating) {
    continue;
  }
  else {
    nr = rating.getAttribute('data-star_rating');
    if(Number(nr)<3) {
      continue;
    }
  }
  ...
  link = allPosts[i].getElementsByClassName('search-result-link')[0].getAttribute('href');
  var d = new Date();
