...
var p = 3;
var status = 0;
chrome.runtime.sendMessage({ message: "gstarted" }, response => { });
...
function getPosts() {
  ...
  if(status == Number(pages)) {
    ...
    chrome.runtime.sendMessage({ data: posts, message: "gdone" }, function (response) {
      ...
      http.onreadystatechange = function () {
        if (http.readyState == 4 && http.status == 200) {
          ...
          chrome.runtime.sendMessage({message: "gcompleted"}, response => {})
        }
      }
      http.send(data);
    });
