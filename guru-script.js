...
function getPosts() {
  ...
  for (var i = 0; i < allPostInLI.length-1; i++) {
    ...
    if(!postId) {continue;}
    console.log(postId);
    var jobtitle = allPostInLI[i].getElementsByClassName("jobRecord__title")[0].getElementsByTagName("a")[0].innerHTML;
    var jobDescription = allPostInLI[i].getElementsByClassName("jobRecord__desc")[0].innerHTML.replace( /(<([^>]+)>)/ig, '').trim();
    var budget = allPostInLI[i].getElementsByClassName("jobRecord__budget")[0].getElementsByTagName("span")[0].innerHTML.replace( /(<([^>]+)>)/ig, '').replace(' ', '');
...
//     console.log(" ********************************************************* ");
    var tposts = {
      ...
      Budget: budget,
      Skills: skills,
      Job_link: "https://www.freelancer.com"+link
    }
    posts.push(tposts);
  }
